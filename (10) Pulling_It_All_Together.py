...
from Classes.error_handling import *
...
displayIT = config['Production']['displayIT']
notifyIT = config['Production']['notifyIT']
logIT = config['Production']['logIT']
...
try:
    ...
    # Load variable with record count of DF for logging
    record_count = final.shape[0]
    
    # Write final to SQL Server
    write_sql(final, 'Breweries', conn_string)

except Exception as error:
    log_to_sql('Importing Brewery Data to SQL', 'Breweries', 'Error Message: ' + error, 0, 2, conn_string)
    error_handling._err(exception = error, display = displayIT, log = logIT, notify = notifyIT, connection_string = conn_string)
else:
    log_to_sql('Importing Brewery Data to SQL', 'Breweries', 'File Imported Correctly', record_count, 1, conn_string)
    error_handling._err(exception = '', display = displayIT, log = logIT, notify = notifyIT, connection_string = conn_string)